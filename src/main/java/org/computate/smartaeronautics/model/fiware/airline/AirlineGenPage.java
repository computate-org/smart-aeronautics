 ...
 * Generated: true
 **/
public class AirlineGenPage extends AirlineGenPageGen<PageLayout> {

  /**
   * {@inheritDoc}
   * Ignore: true
   **/
  protected void _searchListAirline_(Wrap<SearchList<Airline>> w) {
  }

  @Override
  protected void _pageResponse(Wrap<String> w) {
    if(searchListAirline_ != null)
      w.o(Optional.ofNullable(searchListAirline_.getResponse()).map(response -> JsonObject.mapFrom(response).toString()).orElse(null));
  }

  @Override
  protected void _stats(Wrap<SolrResponse.Stats> w) {
    w.o(Optional.ofNullable(searchListAirline_.getResponse()).map(response -> response.getStats()).orElse(null));
  }

  @Override
  protected void _facetCounts(Wrap<SolrResponse.FacetCounts> w) {
    w.o(Optional.ofNullable(searchListAirline_.getResponse()).map(response -> response.getFacetCounts()).orElse(null));
  }

  @Override
  protected void _pagination(JsonObject pagination) {
    JsonArray pages = new JsonArray();
    Long start = searchListAirline_.getStart().longValue();
    Long rows = searchListAirline_.getRows().longValue();
    Long foundNum = Optional.ofNullable(searchListAirline_.getResponse()).map(response -> response.getResponse().getNumFound().longValue()).orElse(Long.valueOf(searchListAirline_.getList().size()));
  ...
  protected void _varsQ(JsonObject vars) {
    Airline.varsQForClass().forEach(var -> {
      JsonObject json = new JsonObject();
      json.put("var", var);
      json.put("displayName", Optional.ofNullable(Airline.displayNameAirline(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("classSimpleName", Optional.ofNullable(Airline.classSimpleNameAirline(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("val", Optional.ofNullable(searchListAirline_.getRequest().getQuery()).filter(fq -> fq.startsWith(Airline.varIndexedAirline(var) + ":")).map(s -> SearchTool.unescapeQueryChars(StringUtils.substringAfter(s, ":"))).orElse(null));
      vars.put(var, json);
    });
  }
  ...
  protected void _varsFq(JsonObject vars) {
    ...
    varsFqCount = 0;
    for(String var : Airline.varsFqForClass()) {
      String varIndexed = Airline.varIndexedAirline(var);
      String varStored = Airline.varStoredAirline(var);
      JsonObject json = new JsonObject();
      json.put("var", var);
      ...
      json.put("displayName", Optional.ofNullable(Airline.displayNameAirline(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("classSimpleName", Optional.ofNullable(Airline.classSimpleNameAirline(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      Object v = searchListAirline_.getRequest().getFilterQueries().stream().filter(fq -> fq.startsWith(Airline.varIndexedAirline(var) + ":")).findFirst().map(s -> SearchTool.unescapeQueryChars(StringUtils.substringAfter(s, ":"))).orElse(null);
      if(v != null) {
        Matcher mFq = Pattern.compile("(\\w+):(.+?(?=(\\)|\\s+OR\\s+|\\s+AND\\s+|$)))").matcher(SearchTool.unescapeQueryChars((String)v));
        StringBuffer sb = new StringBuffer();
        while(mFq.find()) {
          String entityVar = Airline.searchVarAirline(varIndexed);
  ...
  protected void _varsRange(JsonObject vars) {
    Airline.varsRangeForClass().forEach(var -> {
      String varIndexed = Airline.varIndexedAirline(var);
      JsonObject json = new JsonObject();
      json.put("var", var);
      json.put("displayName", Optional.ofNullable(Airline.displayNameAirline(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("classSimpleName", Optional.ofNullable(Airline.classSimpleNameAirline(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("val", searchListAirline_.getRequest().getFilterQueries().stream().filter(fq -> fq.startsWith(Airline.varIndexedAirline(var) + ":")).findFirst().map(s -> SearchTool.unescapeQueryChars(StringUtils.substringAfter(s, ":"))).orElse(null));
      vars.put(var, json);
    });
  }
  ...
  protected void _query(JsonObject query) {
    ...
    Long num = Optional.ofNullable(searchListAirline_.getResponse()).map(response -> response.getResponse().getNumFound().longValue()).orElse(Long.valueOf(searchListAirline_.getList().size()));
    String q = "*:*";
    ...
    Long rows1 = Optional.ofNullable(searchListAirline_).map(l -> l.getRows()).orElse(10L);
    Long start1 = Optional.ofNullable(searchListAirline_).map(l -> l.getStart()).orElse(1L);
    ...
    for(String fq : Optional.ofNullable(searchListAirline_).map(l -> l.getFilterQueries()).orElse(Arrays.asList())) {
      if(!StringUtils.contains(fq, "(")) {
        String fq1 = Airline.searchVarAirline(StringUtils.substringBefore(fq, ":"));
        String fq2 = StringUtils.substringAfter(fq, ":");
        if(!StringUtils.startsWithAny(fq, "classCanonicalNames_", "archived_", "sessionId", "userKeys"))
          fqs.put(fq1, new JsonObject().put("var", fq1).put("val", fq2).put("displayName", Airline.displayNameForClass(fq1)));
        }
      }
    query.put("fq", fqs);

    JsonArray sorts = new JsonArray();
    for(String sort : Optional.ofNullable(searchListAirline_).map(l -> l.getSorts()).orElse(Arrays.asList())) {
      String sort1 = Airline.searchVarAirline(StringUtils.substringBefore(sort, " "));
      sorts.add(new JsonObject().put("var", sort1).put("order", StringUtils.substringAfter(sort, " ")).put("displayName", Airline.displayNameForClass(sort1)));
    }
  ...
  protected void _rangeGap(Wrap<String> w) {
    if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.gap", null) != null)
      w.o(Optional.ofNullable(searchListAirline_.getFacetRangeGap()).orElse(null));
  }
  ...
  protected void _rangeEnd(Wrap<ZonedDateTime> w) {
    if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.end", null) != null)
      w.o(Optional.ofNullable(searchListAirline_.getFacetRangeEnd()).map(s -> TimeTool.parseZonedDateTime(defaultTimeZone, s)).orElse(null));
  }
  ...
  protected void _rangeStart(Wrap<ZonedDateTime> w) {
    if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.start", null) != null)
      w.o(Optional.ofNullable(searchListAirline_.getFacetRangeStart()).map(s -> TimeTool.parseZonedDateTime(defaultTimeZone, s)).orElse(null));
  }
  ...
  protected void _defaultRangeVar(Wrap<String> w) {
    w.o(Optional.ofNullable(searchListAirline_.getFacetRanges()).orElse(Optional.ofNullable(defaultRangeStats).map(s -> Arrays.asList(s.getString("defaultRangeVar"))).orElse(Arrays.asList())).stream().findFirst().map(v -> { if(v.contains("}")) return StringUtils.substringBefore(StringUtils.substringAfterLast(v, "}"), "_"); else return Airline.searchVarAirline(v); }).orElse("created"));
  }

  @Override
  protected void _defaultFacetSort(Wrap<String> w) {
    w.o(Optional.ofNullable(searchListAirline_.getFacetSort()).orElse("index"));
  }

  @Override
  protected void _defaultFacetLimit(Wrap<Integer> w) {
    w.o(Optional.ofNullable(searchListAirline_.getFacetLimit()).orElse(1));
  }

  @Override
  protected void _defaultFacetMinCount(Wrap<Integer> w) {
    w.o(Optional.ofNullable(searchListAirline_.getFacetMinCount()).orElse(1));
  }

  @Override
  protected void _defaultPivotMinCount(Wrap<Integer> w) {
    w.o(Optional.ofNullable(searchListAirline_.getFacetPivotMinCount()).orElse(0));
  }

  @Override
  protected void _DEFAULT_MAP_LOCATION(Wrap<JsonObject> w) {
    Point point = Airline.staticSetLocation(siteRequest_, Optional.ofNullable(siteRequest_.getRequestVars().get(VAR_DEFAULT_MAP_LOCATION)).orElse(siteRequest_.getConfig().getString(ConfigKeys.DEFAULT_MAP_LOCATION)));
  ...
  protected void _defaultSortVars(List<String> l) {
    if(!searchListAirline_.getDefaultSort()) {
      Optional.ofNullable(searchListAirline_.getSorts()).orElse(Arrays.asList()).forEach(varSortStr -> {
        String varSortParts[] = varSortStr.split(" ");
        String varSort = Airline.searchVarAirline(varSortParts[0]);
  ...
  protected void _defaultFieldListVars(List<String> l) {
    Optional.ofNullable(searchListAirline_.getFields()).orElse(Arrays.asList()).forEach(varStored -> {
      ...
      for(String part : parts) {
        if(StringUtils.isNotBlank(part)) {
          String var = Airline.searchVarAirline(part);
  ...
  protected void _defaultStatsVars(List<String> l) {
    Optional.ofNullable(searchListAirline_.getStatsFields()).orElse(Arrays.asList()).forEach(varIndexed -> {
  ...
  protected void _defaultPivotVars(List<String> l) {
    Optional.ofNullable(searchListAirline_.getFacetPivots()).orElse(Arrays.asList()).forEach(facetPivot -> {
   ...
   * {@inheritDoc}
   **/
  protected void _listAirline(JsonArray l) {
    Optional.ofNullable(searchListAirline_).map(o -> o.getList()).orElse(Arrays.asList()).stream().map(o -> JsonObject.mapFrom(o)).forEach(o -> l.add(o));
  }

  protected void _resultCount(Wrap<Integer> w) {
    w.o(searchListAirline_ == null ? 0 : searchListAirline_.size());
  }

  /**
   * Initialized: false
  **/
  protected void _result(Wrap<Airline> w) {
  ...
  protected void _pageTitle(Wrap<String> c) {
    if(result != null && result.getObjectTitle() != null)
      ...
      c.o("Airlines");
    else if(searchListAirline_ == null || resultCount == 0)
      c.o("no Airline found");
    else
      c.o("Airlines");
  }
  ...
  protected void _pageUri(Wrap<String> w) {
    if("enUS".equals(lang))
      w.o("/en-us/search/airline");
  }

  @Override
  protected void _apiUri(Wrap<String> w) {
    w.o("/en-us/api/airline");
  }
  ...
  protected void _pageDescription(Wrap<String> c) {
      c.o("A description of a generic airline");
  }

  @Override
  protected void _pageImageUri(Wrap<String> c) {
      c.o("/png/en-us/search/airline-999.png");
  }
  ...
  protected void _pageUriAirline(Wrap<String> c) {
      c.o("/en-us/search/airline");
  }
}
